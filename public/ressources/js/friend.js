...
const textPlayer = document.getElementById("script").getAttribute("textPlayer");
const textLeft = document.getElementById("script").getAttribute("textLeft");
...
socket.onmessage = function(e) {
	...
	if (message === 'latency') return latencyResponse();

	if (typeof message['exit'] !== 'undefined') return exitGame(message['exit']);

	if (typeof message['message'] !== 'undefined') return newMessage(message['message']);

	if (typeof message['player'] !== 'undefined') newPlayer(message['player']);
	...
	if (typeof message['last_phase'] !== 'undefined') {
		if (message['last_phase']) {
			replay(false);
			plus(true);

			newPlayer(true);

			setTimeout(setUsedDices(), 50);
		}
		else {
			replay(true);
			plus(false);
...
socket.onclose = function() {
	...
	window.location.replace("/pregame/friend");
}
socket.onerror = function(error) {
	var xhttp = new XMLHttpRequest();
	xhttp.open("GET", "/call/game/socket.error.php?q=" + encodeURI(JSON.stringify(error)), true);
...
function latencyResponse() {
	response = Math.round(window.performance.now());

	ping = response - latencyBegin;
	document.getElementById('ping').innerHTML = ping;
}

function latencySend() {
	latencyBegin = Math.round(window.performance.now());

	socket.send('"latency"');
}

var countUnread = 0;
var navOpenClosed = false;

function openNav() {

	navOpenClosed = true;

	e = document.getElementById('mySidebar');
	document.getElementById('undermySidebar').style.display = "block";
	e.style.display = "block";

	document.getElementById('img-message').innerHTML = "<img src=\"/ressources/img/chat.svg\">";
	countUnread = 0;
	...
	setTimeout(function() {
		...
		obj = document.getElementById("mySidebar");
...
function closeNav() {
	...
	setTimeout(function() {
		document.getElementById("undermySidebar").style.display = "none";
...
function exitGame(username) {

	socket.onclose = function () {};
	socket.close();

	alert(textLeft.replace("-{player}-", username));
	window.location.replace("/pregame/friend");
}
...
function sendMessage() {
	...
	document.getElementById('all-message').innerHTML += "<p><strong>You:&nbsp;</strong>" + message + "<br/></p>";
...
function newMessage(argument) {
	
	document.getElementById('all-message').innerHTML += "<p><strong>" + argument['sender'] + ":&nbsp;</strong>" + argument['text'] + "<br/></p>";

	obj = document.getElementById("mySidebar");
	obj.scrollTop = obj.scrollHeight;
	
	if (navOpenClosed) return;

	countUnread++;

	e = document.getElementById("img-message");
	e.innerHTML = "<span class=\"badge bg-danger px-2\">" + countUnread + "</span>";
	e.classList.remove('animate__tada');
	e.offsetWidth;
	e.classList.add('animate__tada');
}

function newPlayer(argument) {
	...
	if (typeof argument === "boolean") {
		document.getElementById('myModal_player').style.display = 'none';
		document.getElementById('text_player').innerHTML = '';
		activePlayer = true;
		return;
	}
	...
	document.getElementById('myModal_player').style.display = 'block';
	document.getElementById('text_player').innerHTML = textPlayer.replace("-{player}-", argument);
}
...
function finish(argument) {
	...
	setTimeout(function() {
		window.location.replace("/result/friend");
	}, 10000);
}
...
function setUsedDices(argument = false) {

	if (argument === false) {
		argument = savedUsedDices;
	}
	else {
		savedUsedDices = argument;
	}
